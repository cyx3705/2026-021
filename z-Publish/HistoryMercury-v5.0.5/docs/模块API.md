...
跨项目读取本模块说明书：先执行 diana.docs.catalog，再执行 diana.docs.read domain=mercury file=docs/模块API.md。

## 开发更新与观察

模块开发使用同目录 Console CLI 的 vulcan.dev.start / submit / finish，流程以宿主发布的模块开发手册为准。
submit 必须指定 start 返回的 worktree，第一轮候选通过后仍需审批才能 finish。

使用 Diana 2.6.0 或更高版本时，改之前执行 `diana.host.observe name=HistoryMercury` 并保留 baselineToken。
submit 回执含「已热重载到活宿主」后，执行 `diana.host.observe name=HistoryMercury baseline=<原令牌>`。
